...
def gf_pow(x, power):
    #print(f"(gf_log[x] * power) = {(gf_log[x] * power)}")
    #print(f"(gf_log[x] * power) % field_charac = {(gf_log[x] * power) % field_charac}")
    #print(f" inverse = {gf_exp[(gf_log[x] * gf_inverse(power))]}")
        ...
        if qj != 0: # log(0) is undefined, we need to check that
            ...
            for i in xrange(len(p)):
                if p[i] != 0: # log(0) is undefined, need to check that...
                    ...
                    #print(f"[{i}][{j}]PY_GF_MULT({p[i]}, {q[j]}) == {gf_mul(p[i], q[j])}")
    return r
    ...
    for i in xrange(len(dividend) - (len(divisor)-1)):
        ...
        if coef != 0: # log(0) is undefined, so we need to avoid that case explicitly (and it's also a good optimization).
            for j in xrange(1, len(divisor)): # in synthetic division, we always skip the first coefficient of the divisior,
                ...
                #print(f"divisor[j] = {divisor[j]}")
                if divisor[j] != 0: # log(0) is undefined
                    msg_out[i + j] ^= gf_mul(divisor[j], coef) # equivalent to the more mathematically correct
                    #print(f"msg_out[{i}+{j}] = {msg_out[i+j]}")# (but xoring directly is faster): msg_out[i + j] += -divisor[j] * coef
...
def gf_poly_eval(poly, x):
    ...
    #print(f"poly = {poly} x = {x}")
    y = poly[0]
    for i in xrange(1, len(poly)):
        #print(f"{i} : gf_mul({y}, {x}) = {gf_mul(y, x)}")        
        y = gf_mul(y, x) ^ poly[i]
        #print(f"{i} : xor = {gf_mul(y, x)} ^ {poly[i]} = {y}")
    return y
...
def rs_correct_errata(msg_in, synd, err_pos, fcr=0, generator=2): # err_pos is a list of the positions of the errors/erasures/errata
    ...
    #print(f"coef_pos = {coef_pos}")
    err_loc = rs_find_errata_locator(coef_pos, generator)
    #print(f"err_loc = {err_loc} = {len(err_loc)-1}")
    ...
    #print(f"__field_charac = {field_charac} X = {X}")
    ...
    for i, Xi in enumerate(X):
        ...
        for j in xrange(Xlength):
            if j != i:
                ...
                ##print(f"gf_mul(Xi_inv = {Xi_inv}, X[j] = {X[j]}) = {gf_mul(Xi_inv, X[j])}")
        ...
        #print(f"err_loc_prime = {err_loc_prime}")
        ...
        #print(f"gf_poly_eval({err_eval[::-1]}, {Xi_inv}) = {y}")
        y = gf_mul(gf_pow(Xi, 1-fcr), y) # adjust to fcr parameter
        #print(f"gf_pow({Xi}, 1-{fcr}) = {gf_pow(Xi, 1-fcr)}")
        #print(f"gf_mul(gf_pow({Xi}, 1-{fcr}), y) = {y}")
        ...
        magnitude_l.append(magnitude)
        #print(f"magnitude = gf_div({y}, {err_loc_prime}) = {magnitude}")
        E[err_pos[i]] = magnitude # store the magnitude for this error into the magnitude polynomial
        #print(f"E[{err_pos[i]}] = {magnitude}")
    ...
    return msg_in, magnitude_l

# nsym = number of roots
def rs_find_error_locator(synd, nsym, erase_loc=None, erase_count=0):
    ...
    #print(f"synd {synd}")
    #print(f"len(synd) / nsym {len(synd)} {nsym}")
    for i in xrange(nsym-erase_count): # generally: nsym-erase_count == len(synd), except when you input a partial erase_loc and using the full syndrome instead of the Forney syndrome, in which case nsym-erase_count is more correct (len(synd) will fail badly with IndexError).
        ...
        for j in xrange(1, len(err_loc)):
            ##print(f"err_loc/synd = {err_loc[-(j+1)]:x} / {synd[K - j]:x}")
        ...
        if delta != 0: # Update only if there's a discrepancy
           ...
           # Update with the discrepancy
            dbg_str = "err_loc_inrm : "
            for item in err_loc:
               dbg_str = dbg_str + f"0x{item:x} "
            #print(dbg_str)
            dbg_str = "old_loc_inrm : "
            for item in old_loc:
               dbg_str = dbg_str + f"0x{item:x} "
            #print(dbg_str)
            err_loc = gf_poly_add(err_loc, gf_poly_scale(old_loc, delta))
        #print(f"len(err_loc) = {len(err_loc)-1}")
        dbg_str = "err_loc : "
        for item in err_loc:
            dbg_str = dbg_str + f"0x{item:x} "
        #print(dbg_str)
...
def rs_find_errata_locator(e_pos, generator=2):
    ...
    #print(f"e_loc = {e_loc}")
    for i in e_pos:
        #print(f"gf_pow({generator}, {i}) = {gf_pow(generator, i)}")
        #print(f"gf_poly_add = {gf_poly_add([1], [gf_pow(generator, i), 0])}")
        e_loc = gf_poly_mul( e_loc, gf_poly_add([1], [gf_pow(generator, i), 0]) )
        #print(f"{i}: e_loc = {e_loc}")
    #print(f"e_loc = {e_loc}")
    return e_loc
...
def rs_find_error_evaluator(synd, err_loc, nsym):
    ...
    #print(f"RS_FIND_ERROR_EVALUATOR")
    #print(f"synd = {synd}")
    #print(f"err_loc = {err_loc}")
    #print(f"nsym = {nsym}")
    #print(f"gf_poly_mul(synd, err_loc) = {gf_poly_mul(synd, err_loc)}")
    #print(f"gf_poly_mul(synd, err_loc) = {gf_poly_mul(synd, err_loc)[0]}")
    #print(f"([1] + [0]*(nsym+1)) = {([1] + [0]*(nsym+1))}")
