...
from rs_lib import RsErrPosPacket
...
async def rs_syndrome_test(dut):
    ...
    s_mon     = AxisMonitor('s_mon', s_if, port_in, T_LEN+1, 0, 1)
    m_mon     = AxisMonitor('m_mon', m_if, pkt_comp.port_out, 1, 0)
    ...
    corrupt_words_num = random.randint(T_LEN-2, T_LEN)
    ...
    pkt = RsErrLocatorPacket(roots_num=ROOTS_NUM, word_size=BUS_WIDTH_IN_SYMB)
    pkt.generate(ref_pkt=ref_msg)
    pkt.print_pkt("ERR_LOCATOR_PKT")

    err_pkt = RsErrPosPacket(roots_num=ROOTS_NUM, word_size=BUS_WIDTH_IN_SYMB)
    err_pkt.generate(ref_pkt=ref_msg)
    err_pkt.print_pkt("ERR_LOCATION_PKT")
