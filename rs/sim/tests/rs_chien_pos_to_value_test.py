...
class RsChienPosToValBaseTest(RsTest):
    ...
    def build_env(self):
        ...
        self.s_drv     = AxisDriver('s_drv', self.s_if, ROOTS_PER_CYCLE__BYTES, 0)
        self.m_mon     = AxisMonitor(name='m_mon', axis_if=self.m_if, aport=self.pkt_comp.port_out, width=T_LEN, tdata_unpack=1)
...
class  RsChienPosToValRandomTest(RsChienPosToValBaseTest):
    ...
    def gen_stimilus(self):
        ...
        for i in range(self.pkt_num):            
            ...
            enc_msg = RsPacket(name=f'enc_msg{i}', n_len=N_LEN, roots_num=ROOTS_NUM, word_size=BUS_WIDTH_IN_SYMB, corrupt_words_num=0)
            enc_msg.generate(ref_pkt=ref_msg)
            
            cor_msg = RsPacket(name=f'cor_msg{i}', n_len=N_LEN, roots_num=ROOTS_NUM, word_size=BUS_WIDTH_IN_SYMB, corrupt_words_num=corrupt_words_num)
            cor_msg.generate(ref_pkt=ref_msg)
        
            pkt = RsErrBitPositionPacket(name=f'err_bit_pos{i}', n_len=N_LEN, roots_num=ROOTS_NUM, word_size=BUS_WIDTH_IN_SYMB)
            pkt.generate(ref_pkt=cor_msg)
        
            err_pos_pkt = RsErrPositionPacket(name=f'err_pos{i}', n_len=N_LEN, roots_num=ROOTS_NUM, word_size=BUS_WIDTH_IN_SYMB)
